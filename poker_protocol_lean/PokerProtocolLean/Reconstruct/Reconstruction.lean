...
import PokerProtocolLean.Reconstruct.ResidualCarrierProvenance
...
2. **residual-carrier binding:** every `-cards[i]` branch is in one-to-one
   correspondence with an authenticated prior-hand residual_carrier ciphertext for
   `cards[i]`.
...
structure Statement (n k : ℕ) where
  ...
  residualCarriers : Fin k → ElGamalCiphertext G

/-- Fail-closed public-statement conditions corresponding to the Rust
validator. Historical authentication of `residualCarriers` is deliberately not
included here; it is supplied by `ResidualCarrierProvenance` and the state root. -/
def WellFormedStatement {n k : ℕ} (stmt : Statement G n k) : Prop :=
  ...
  (∀ j, (stmt.residualCarriers j).c1 ≠ 0 ∧ (stmt.residualCarriers j).c2 ≠ 0)

/-- Extractable witness for the repaired statement.

`residualCarrierIndex` is injective, and `removed_iff_residual_carrier` gives exact coverage:
there are no duplicate residual carriers and no unbacked removal branch. -/
structure Witness (n k : ℕ) where
  removed : Fin n → Bool
  contributionRandomness : Fin n → F
  residualCarrierIndex : Fin k → Fin n
  residualRandomness : Fin k → F
  residualCarrierIndex_injective : Function.Injective residualCarrierIndex
  removed_iff_residual_carrier : ∀ i, removed i = true ↔ ∃ j, residualCarrierIndex j = i
...
def Relation {n k : ℕ} (stmt : Statement G n k) (wit : Witness F n k) : Prop :=
  ...
  (∀ j, stmt.residualCarriers j =
    ElGamalCiphertext.encrypt F G stmt.g
      (stmt.cards (wit.residualCarrierIndex j)) stmt.ownerPk
      (wit.residualRandomness j))
...
theorem relation_complete {n k : ℕ}
    ...
    (hcontribution : ∀ i, stmt.contributions i =
      ElGamalCiphertext.encrypt F G stmt.g
        (contributionMessage G (stmt.cards i) (wit.removed i))
        stmt.aggregatePk (wit.contributionRandomness i))
    (hresidual : ∀ j, stmt.residualCarriers j =
      ElGamalCiphertext.encrypt F G stmt.g
        (stmt.cards (wit.residualCarrierIndex j)) stmt.ownerPk
        (wit.residualRandomness j)) :
    Relation F G stmt wit :=
  ⟨hcontribution, hresidual⟩
...
theorem valid_relation_complete {n k : ℕ}
  ...
  ⟨hwellFormed, relation_complete F G stmt wit hcontribution hresidual⟩

/-- The residual-carrier half of the relation follows from authenticated
prior-hand provenance, rather than being trusted as arbitrary prover input. -/
theorem residual_carrier_equations_of_authenticated_lineage {n k : ℕ}
    (stmt : Statement G n k) (wit : Witness F n k)
    (hprovenance : ∀ j, ∃ (priorAggregatePk : G) (otherSk : F)
        (dealt : ElGamalCiphertext G),
      ResidualCarrierProvenance.AuthenticatedLineage F G stmt.g
        (stmt.cards (wit.residualCarrierIndex j)) priorAggregatePk
        (wit.residualRandomness j) dealt ∧
      priorAggregatePk = stmt.ownerPk + otherSk • stmt.g ∧
      stmt.residualCarriers j =
        ResidualCarrierProvenance.removeOtherPlayers F G otherSk dealt) :
    ∀ j, stmt.residualCarriers j =
      ElGamalCiphertext.encrypt F G stmt.g
        (stmt.cards (wit.residualCarrierIndex j)) stmt.ownerPk
        (wit.residualRandomness j) := by
  intro j
  rcases hprovenance j with ⟨priorAggregatePk, otherSk, dealt,
    hlineage, haggregate, hresidual⟩
  exact ResidualCarrierProvenance.authenticated_prior_hand_yields_residual_carrier
    F G hlineage haggregate hresidual
...
of the authenticated residual carriers. -/
theorem removed_iff_has_residual_carrier_witness {n k : ℕ}
    (wit : Witness F n k) (i : Fin n) :
    wit.removed i = true ↔ ∃ j, wit.residualCarrierIndex j = i :=
  wit.removed_iff_residual_carrier i

/-- Residual-carrier indices cannot be duplicated. -/
theorem residual_carrier_indices_are_unique {n k : ℕ}
    (wit : Witness F n k) {j₁ j₂ : Fin k}
    (h : wit.residualCarrierIndex j₁ = wit.residualCarrierIndex j₂) : j₁ = j₂ :=
  wit.residualCarrierIndex_injective h
...
contribution encrypts the negative of an owner-residual plaintext.

The witness is `(ownerSk, contributionRandomness)`.  Crucially, it does not
contain the residual carrier's ElGamal exponent and therefore does not require
knowing `DL(residual_carrier.c1)`. -/
def CrossKeyNegationRelation
    (g ownerPk aggregatePk : G)
    (residual_carrier contribution : ElGamalCiphertext G)
    (ownerSk contributionRandomness : F) : Prop :=
  ownerPk = ownerSk • g ∧
  contribution.c1 = contributionRandomness • g ∧
  ownerSk • residual_carrier.c1 + contributionRandomness • aggregatePk =
    residual_carrier.c2 + contribution.c2
...
theorem cross_key_negation_complete
    (g ownerPk aggregatePk m : G) (ownerSk residualRandomness contributionRandomness : F)
    (hpk : ownerPk = ownerSk • g) :
    CrossKeyNegationRelation F G g ownerPk aggregatePk
      (ElGamalCiphertext.encrypt F G g m ownerPk residualRandomness)
...
/-- Soundness consequence of the joint relation: decrypting the residual carrier
...
theorem cross_key_negation_binds_plaintexts
    ...
    (hrel : CrossKeyNegationRelation F G g ownerPk aggregatePk
      residual_carrier contribution ownerSk contributionRandomness) :
    (residual_carrier.c2 - ownerSk • residual_carrier.c1) +
  ...
  calc
    (residual_carrier.c2 - ownerSk • residual_carrier.c1) +
      ...
      (residual_carrier.c2 + contribution.c2) -
        (ownerSk • residual_carrier.c1 + contributionRandomness • aggregatePk) := by abel
